public class DifMatrixMultiplication
{
    ...
    public double[][] multiply(double[][] A, double[][] B, int cores) throws InterruptedException {
        int n = A.length;
        ...
        for(int w=0;w<cores;w++)
        {
            ...
            if (w==cores-1) // last thread may get more rows
                Ra += n%cores;
            workercoreD wc = new workercoreD(A,B,R,Rs*w,Ra);
            Ts[w] = new Thread(wc);
            Ts[w].start(); // start right away
        }
        for(int w=0;w<cores;w++) Ts[w].join(); // wait for each to finish
...
class workercore implements Runnable
{
    ...
    public workercore(int[][] a, int[][] b, int[][] c, int s, int n) {A=a; B=b; R=c; start=s; N=n;}
...
class workercoreD implements Runnable
{
    double[][] A;  // pointer to external matrices, AXB = R
    double[][] B;
    double[][] R;
    int start, N; // starting row and number of rows.
    public workercoreD(double[][] a, double[][] b, double[][] c, int s, int n) {A=a; B=b; R=c; start=s; N=n;}
